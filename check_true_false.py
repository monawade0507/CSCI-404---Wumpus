...
def main(argv):
    ...
    for line in input_file:
        ...
        if subexpression.connective[0] == '':
            list[subexpression.symbol[0]] = True
        if subexpression.connective[0].upper() == 'NOT':
            if subexpression.subexpressions[0].symbol and subexpression.subexpressions[0].connective[0] == '':
                list[subexpression.subexpressions[0].symbol[0]] = False
    ...
    try:
        ...
        sys.exit(0)
    print '\nLoading statement...'
    statement_str = input_file.readline().rstrip('\r\n')
    input_file.close()

    # Convert statement into a logical expression and verify it is valid
    statement_expr = read_expression(statement_str)
    if not valid_expression(statement_expr):
        sys.exit('invalid statement')

    # Show us what the statement is
    print '\nChecking statement: ',
    print_expression(statement_expr, '')
    print '\n'


    # Run the statement through the inference engine
    check_true_false(knowledge_base, statement_expr)
